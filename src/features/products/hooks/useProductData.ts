...
export function useProductData(productId: string | number): UseQueryResult<Product | null, Error>;
export function useProductData(options: ProductDataOptions): UseQueryResult<Product[], Error>;
...
export function useProductData(
  ...
  const fetchFeaturedProducts = async (limit?: number): Promise<Product[]> => {
    const response: ApiResponse<Product[]> = await productService.getFeatured(limit);
    if (response.error) {
      throw new Error(response.error);
    }
    return response.data || [];
  ...
  if (options?.isFeatured) {
    return featuredProductsQuery as UseQueryResult<Product | Product[] | null, Error>;
  }
