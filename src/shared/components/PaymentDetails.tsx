...
const PaymentDetails = () => {
  ...
  const selectedServicesDetails = services.filter((service) =>
    orderState.selectedServices.includes(service.id)
  );

  const selectedProductsDetails = products.filter((product) =>
    orderState.selectedProducts.includes(product.id)
  );

  // Find staff assigned to each service
  const getServiceProvider = (serviceId: number) => {
    return orderState.serviceProviders?.find((sp) => sp.serviceId === serviceId)
      ?.name;
  };
                ...
                <AccordionItem value="services" className="border-none">
                  <div className="flex justify-between items-center">
                    ...
                    <AccordionTrigger className="py-0">
                      <span className="text-sm text-glamour-700">
                        View Details
                      </span>
                    </AccordionTrigger>
                  </div>
                  <AccordionContent>
                    <div className="space-y-4 mt-2">
                      ...
                      <div>
                        <h4 className="text-sm font-semibold mb-2 flex items-center">
                          <Calendar className="h-4 w-4 mr-1" /> Appointment
                          Details
                        </h4>
                  ...
                  {showBankDetails && (
                    <div className="border-t p-4 bg-gray-50">
                      ...
                      <p className="text-xs text-muted-foreground mt-3">
                        Please include your name and appointment date in the
