...
export function useProducts() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const data = await api.execute(() => productService.getAll(), {
  ...
  const getProduct = useCallback(async (id: number | string) => {
    ...
    const response = await productService.getById(numericId);
    return response.data;
  }, []);

  const getProductsByService = useCallback(
    async (serviceId: number | string) => {
      const numericId =
        typeof serviceId === "string" ? parseInt(serviceId, 10) : serviceId;
      const response = await productService.getByServiceId(numericId);
      return response.data || [];
    },
    []
  );
  ...
  const createProduct = useCallback(
    async (data: ProductFormData) => {
      const result = await api.execute(() => productService.create(data), {
  ...
  const updateProduct = useCallback(
    async (id: number | string, data: Partial<ProductFormData>) => {
      const numericId = typeof id === "string" ? parseInt(id, 10) : id;
      const result = await api.execute(
        () => productService.update(numericId, data),
        {
          showSuccessToast: true,
  ...
  const deleteProduct = useCallback(
    async (id: number | string) => {
      const numericId = typeof id === "string" ? parseInt(id, 10) : id;
      const result = await api.execute(() => productService.delete(numericId), {
        showSuccessToast: true,
